...
if sync == 58
%% read chip id
[crc_error_count]=ReadScCrcErrorCount(1)
[~,~,id]= read_register(hex2dec('00010003')) ;
 chip_id = strcat('vfat3-',num2str(id));
...
set_preamp_simple(PT,Gain,sc_hw);
%%
...
[scurve,mean_th,mean_enc,~,~,~,~] = scurve_all_channel_updated(start_chan,stop_chan,step_chan,Latency,LV1As,arm_dac,D1,D2,DELAY,start_fc,stop_fc,fc_arr,dac,calpulse);
%%
...
[crc_error_count]=ReadScCrcErrorCount(0)
%%
% S_scurve_B(1,:,:)        = scurve;
