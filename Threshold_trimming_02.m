...
if(Threshold(channel+1) > M_O_mean_Th) % if channel threshold is greater than mean
    DATA=bitset(DATA,7,'uint16');% make 7th bit to 1 
else 
    DATA = bitand(DATA,hex2dec('ffbf'),'uint16');%clear the bit
end
while ch_trim ~= YES 
...
%%
if(DATA ==63 || DATA == 127 )% +ve max value or -ve max value
    ch_trim =YES;
    
else
    
DATA = DATA +1;% INCREASE THE CHANNEL ARM TRIMMING DAC BY ONE
