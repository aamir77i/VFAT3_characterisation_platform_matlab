%% Routine to measure slow_control_rw_histograms 
% 
%  
% 
% 
...
read_data=typecast(temp, 'uint32');
%read_data = reshape(temp,[1,1000*NUM_OF_READS]);
fclose(t);
%% 
...
figure();
histogram(read_data,5);

xmin =  (min(read_data)- (min(read_data))/10);
xmax = (max(read_data) + (max(read_data)/10));%10 % more axis scale
ymin =  0;
ymax = (1.2 *1000*NUM_OF_READS);% 20 % more y scale

axis([xmin   xmax ymin ymax] );


str0 = ['',VFAT3_NUMBER];
str1 = ['Total # of Reads:',num2str(NUM_OF_READS*1000)];
